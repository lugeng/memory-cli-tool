...
async def read_note(
    ...
    # Get the file via direct service call - first try direct permalink lookup
    ...
    logger.info(f"Attempting to read note via direct service call: {entity_path}")

    try:
        # Try direct lookup first using direct service call
        ...
        db_entity = await entity_service.get_by_permalink(entity_path)
        
        # If successful, return the content
        if db_entity:
            file_service = services['entity_service'].file_service
            content = await file_service.read_file(db_entity.file_path)
            logger.info(f"Returning read_note result from direct service: {entity_path}")
    ...
    if title_results and title_results.results:
        ...
        if result.permalink:
            try:
                # Try to fetch the content using the found permalink via direct service call
                from basic_memory.mcp.tools.utils import get_direct_services
                services = await get_direct_services(project)
                entity_service = services['entity_service']
                
                # Get entity by permalink
                db_entity = await entity_service.get_by_permalink(result.permalink)
