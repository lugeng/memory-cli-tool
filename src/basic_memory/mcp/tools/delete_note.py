...
async def delete_note(identifier: str, project: Optional[str] = None) -> bool | str:
    ...
    try:
        ...
        try:
            entity = await entity_service.get_by_permalink(identifier)
            if entity is None:
                result = DeleteEntitiesResponse(deleted=False, message="No entity found to delete")
            else:
                # Delete entity by identifier
                deleted = await entity_service.delete_entity(identifier)
                result = DeleteEntitiesResponse(
                    deleted=deleted,
                    message=f"Deleted entity" if deleted else "No entity found to delete"
                )
        except Exception:
            result = DeleteEntitiesResponse(deleted=False, message="No entity found to delete")
