...
async def get_direct_services(project_name: Optional[str] = None):
    """Get direct service instances for CLI operations, bypassing HTTP layer.
    
    This replicates the dependency injection from deps.py for direct service access.
    """
    ...
    if project_name:
        ...
        target_project_name = app_config.default_project
    
    project_repo = ProjectRepository(session_maker)
    project_obj = await project_repo.get_by_permalink(target_project_name)
    if not project_obj:
        project_obj = await project_repo.get_by_name(target_project_name)
    if not project_obj:
        raise ValueError(f"Project '{target_project_name}' not found")
    
    project_id = project_obj.id
    project_config = ProjectConfig(name=project_obj.name, home=pathlib.Path(project_obj.path))
    
    entity_repository = EntityRepository(session_maker, project_id=project_id)
    observation_repository = ObservationRepository(session_maker, project_id=project_id)
    relation_repository = RelationRepository(session_maker, project_id=project_id)
    search_repository = SearchRepository(session_maker, project_id=project_id)
    ...
    link_resolver = LinkResolver(entity_repository=entity_repository, search_service=search_service)
