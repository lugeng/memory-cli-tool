...
async def write_note(
    ...
    # Create or update via direct service call instead of HTTP API
    logger.debug(f"Creating entity via direct service call permalink={entity.permalink}")
    ...
    db_entity, created = await entity_service.create_or_update_entity(entity)
    
    search_service = services['search_service']
    await search_service.index_entity(db_entity)
    
    result = EntityResponse.model_validate(db_entity)
    
    response_status_code = 201 if created else 200
    
    # Format semantic summary based on status code
    action = "Created" if response_status_code == 201 else "Updated"
    summary = [
        f"# {action} note",
    ...
    logger.info(
        f"MCP tool response: tool=write_note action={action} permalink={result.permalink} observations_count={len(result.observations)} relations_count={len(result.relations)} resolved_relations={resolved} unresolved_relations={unresolved} status_code={response_status_code}"
    )
    return "\n".join(summary)
